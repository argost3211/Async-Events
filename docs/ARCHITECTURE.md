...
Use cases **не вызывают** метрики напрямую. Они возвращают доменные сущности или result-объекты (например, `RepublishResult`), содержащие достаточно информации для подсчёта метрик вызывающим кодом на уровне адаптера.
...
- Use case → класс с суффиксом `UseCase` и методом `execute` (например, `RepublishUnpublishedEventsUseCase`).
- Протокол → `typing.Protocol` в `use_cases/protocols.py`.
- Result → `dataclass(frozen=True)` рядом с use case при необходимости (например, `RepublishResult`); иначе use case возвращает доменную сущность.
...
6. Проверить: `api/` не содержит многошаговых сценариев (нескольких шагов), вся логика в `use_cases/` или `services/`, а в `api/` только вызовов